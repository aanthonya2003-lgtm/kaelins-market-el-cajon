...
export function WeeklyAdHero({ featured, totalPosts }: Props) {
  ...
  const [countdown, setCountdown] = useState(() =>
    formatCountdown(nextTuesday6am().getTime() - Date.now())
  );
  ...
  useEffect(() => {
    const tick = () =>
      setCountdown(formatCountdown(nextTuesday6am().getTime() - Date.now()));
  ...
  return (
    <>
                ...
                <div data-hero-sub className="mt-8 flex items-center gap-4 flex-wrap">
                  {featuredHasImage ? (
                    <button
                      ...
                      className="inline-flex items-center gap-3 bg-[var(--color-chile)] text-[var(--color-cream)] px-6 py-4 font-mono tracking-[0.14em] text-sm uppercase hover:bg-[var(--color-ink)] transition-colors duration-300 cursor-zoom-in"
                    >
                      View latest ad →
                    </button>
                  ) : (
      ...
      <Lightbox
        src={lightboxOpen ? featured?.imageUrl ?? null : null}
        alt={featured?.title ?? 'Weekly ad'}
        caption={
          featured?.pubDate
